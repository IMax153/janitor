...
- For HTTP requests, use `HttpClient` and `HttpClientRequest` from `effect/unstable/http`, and provide the client with `Effect.provide(effect, Http.layer)` where `Http` comes from `foldkit`. See `@foldkit/examples/weather/src/main.ts` for the pattern
...
Canonical reference: `@foldkit/examples/form/src/main.ts` (async email uniqueness check with version-based cancellation) and `@foldkit/examples/job-application/src/step/` (validated multi-step forms across submodels).
...
- Canonical reference: `@foldkit/examples/job-application/src/step/attachments.ts`
...
- Always end with `Runtime.run(application)` for a page-owning app. When a host application controls the program's lifecycle, end with `Runtime.embed(element)` instead and hand the returned handle to the host; mirror `@foldkit/examples/embedding/src/host.ts` for the host side and its `main.ts` for the widget side
...
- `@foldkit/examples/weather/src/story.test.ts`: simple Command resolution (happy path + error path)
- `@foldkit/examples/auth/src/page/loggedOut/page/login.story.test.ts`: Submodel with OutMessage assertions, field validation
- `@foldkit/packages/website/src/search/story.test.ts`: multi-step interactions (arrow key cycling, stale result handling)
...
- `@foldkit/examples/weather/src/scene.test.ts`: basic Scene flow with form interaction and Command resolution
- `@foldkit/examples/auth/src/scene.test.ts`: a multi-page app's root-level Scene driving the login flow through the root view
- `@foldkit/examples/auth/src/page/loggedOut/page/login.scene.test.ts`: a page-scoped Scene in that same app, driving the page's own `update`/`view` rather than the root pair
- `@foldkit/examples/kanban/src/scene.test.ts`: scoped queries with `within`, `toHaveValue`, explicit test data
...
code in `@foldkit/packages/typing-game/client/src/` or `@foldkit/packages/website/src/`.
...
5. .agents/skills/generate-program/blindSpots.md
6. At least one exemplar file matching the generated app's complexity
   from the `@foldkit` OpenCode reference:
   - Tier 1-2: @foldkit/packages/foldkit/src/runtime/runtime.ts (quality calibration)
   - Tier 3-4: @foldkit/examples/weather/src/main.ts OR @foldkit/examples/form/src/main.ts
   - Tier 5: @foldkit/examples/kanban/src/update.ts AND @foldkit/examples/kanban/src/domain/
   - Tier 6: @foldkit/examples/job-application/src/update.ts AND @foldkit/examples/auth/src/page/
   - Tier 7: @foldkit/packages/typing-game/client/src/page/room/

Every path above is project-relative or uses the `@foldkit` OpenCode
reference. Do not rewrite them as absolute paths.
