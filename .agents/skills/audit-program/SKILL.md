...
Audit an existing Foldkit program against the same bar `generate-program` targets: this code should be **indistinguishable in quality from hand-written code in `@foldkit/packages/typing-game/client/src/` or `@foldkit/packages/website/src/`**. Not "works." Not "structurally valid." Typing-game quality.

> **Canonical source:** use OpenCode's `@foldkit` reference for live exemplars (`@foldkit/examples/`, `@foldkit/packages/typing-game/`, `@foldkit/packages/website/`). The local architecture and conventions guides are snapshots of those sources.
...
These four files are a snapshot. The live source under `@foldkit` wins over anything here: read the `.d.ts` or the example rather than grading against a remembered signature.
...
4. **Tier-matching exemplar**. Pick at least one to read alongside the audited code:
   - Tier 1-2 → `@foldkit/examples/counter/src/main.ts`, `@foldkit/examples/stopwatch/src/main.ts`
   - Tier 3 → `@foldkit/examples/weather/src/main.ts`, `@foldkit/examples/form/src/main.ts`
   - Tier 4 → `@foldkit/examples/routing/src/main.ts`
   - Tier 5 → `@foldkit/examples/kanban/src/`
   - Tier 6 → `@foldkit/examples/auth/src/`, `@foldkit/examples/job-application/src/`
   - Tier 7 → `@foldkit/packages/typing-game/client/src/page/room/`
5. **Foldkit UI integration**. If anything is imported from `@foldkit/ui`, also read `@foldkit/examples/ui-showcase/src/main.ts` and `@foldkit/examples/ui-showcase/src/ui/` for the wiring pattern.
...
- **Config located**: grep it for `foldkit`. A hit under `extends` pointing at
  ...
  or workspace setup wires it and how the Foldkit repo itself does it, means the
