...
> **Note:** This file is a snapshot of the architecture as practiced in the live Foldkit codebase. If anything here contradicts `@foldkit/examples/` or `@foldkit/packages/foldkit/src/`, the live code is canonical.
...
- **With args, fallible** (HTTP fetch): `@foldkit/examples/weather/src/main.ts` (`FetchWeather`)
- **With args, infallible** (random + return): `@foldkit/examples/kanban/src/command.ts` (`GenerateCardId`)
- **With args, storage**: `@foldkit/examples/kanban/src/command.ts` (`SaveBoard`)
- **Argless, DOM side effect**: `@foldkit/examples/kanban/src/command.ts` (`FocusAddCardInput`)
...
`@foldkit/examples/route-transitions/src/main.ts` uses both.
...
- **Always-active keyboard input**: `@foldkit/examples/snake/src/main.ts`
- **Animation-frame-driven game tick**: `@foldkit/examples/canvas-art/src/main.ts`
- **Conditional WebSocket connection** (active only when a session exists): `@foldkit/examples/websocket-chat/src/main.ts`
- **Production multi-stream pattern**: `@foldkit/packages/typing-game/client/src/subscription.ts`
...
Use these instead of raw `document.querySelector`, `setTimeout`, `Date.now()`, or `Math.random()`. They compose naturally inside `Command.define`. For canonical wiring, see `@foldkit/examples/kanban/src/command.ts` (`FocusAddCardInput` wraps `Dom.focus`) and `@foldkit/examples/stopwatch/src/main.ts` (`Clock.currentTimeMillis` inside an `Effect.gen`).
...
For the canonical update-handler shapes (the exact `UrlRequest` tag names, how to dispatch `pushUrl` vs an external load Command, and how to derive the route from a `Url`), see `@foldkit/examples/routing/src/main.ts`.
