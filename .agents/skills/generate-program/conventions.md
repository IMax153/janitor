...
> **Note:** This file is a snapshot of the conventions practiced in the live Foldkit codebase. If anything here contradicts `@foldkit/examples/` or `@foldkit/packages/foldkit/src/`, the live code is canonical.
...
Command.define("DetermineStartTime", {
  ...
  execute: ({ elapsedMs }) =>
    Clock.currentTimeMillis.pipe(
      Effect.map((now) => Message.CompletedDetermineStartTime({ startTime: now - elapsedMs })),
    ),
})
Command.define("GenerateCardId", {
  ...
  execute: ({ columnId }) =>
    Effect.uuid.pipe(Effect.map((cardId) => Message.CompletedGenerateCardId({ cardId, columnId }))),
})
...
Effect.gen(function* () {
  ...
  Effect.catch((error) => Effect.succeed(Message.FailedFetchWeather({ error: String(error) }))),
  FetchWeather,
)
```
...
pushUrl(path).pipe(Effect.ignore, Effect.as(Message.CompletedNavigateInternal()))
